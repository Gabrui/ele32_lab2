...
public class TesteEscritor {
	...
	public void testBitsMultiploDe8() throws IOException {
		...
		binMap.put(2, "c");
		binMap.put(3, "d");
		Escritor e = new Escritor(binMap, lista);
		...
		assertEquals(0b00001010, bytes[9]);
		ins.close();
	}
	...
	public void testaBits() throws IOException {
		...
		assertEquals((byte)'a', bytes[5]);
		assertEquals((byte)'b', bytes[6]);
		assertEquals(0b01000000, bytes[7]);
		ins.close();
	}
	...
	public void testaBits2() throws IOException {
		...
		assertEquals((byte)'a', bytes[5]);
		assertEquals((byte)'b', bytes[6]);
		assertEquals((byte)0b01000010, bytes[7]);
		assertEquals((byte)0b10000000, bytes[8]);
		ins.close();
	}
